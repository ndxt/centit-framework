...
<html lang="zh-cmn-Hans">
	<head>
    	...
    	<style type="text/css">
    	.red{position: absolute;top: 216px;left: 60%;color: red;}
    	</style>
	</head>
	<body>
		<div class="header">
...
<!--            			<span><span color="red">用户名或者密码错误</span></span> -->
<%--        			</c:if>		 --%>
<%-- 			  </c:if> --%>
		</div>
				<i></i>
				<div style='position:relative;'>
				<c:if test="${error != null && param.error != ''}">
       			<c:choose> 
				  ...
				  <c:when test="${SPRING_SECURITY_LAST_EXCEPTION.message eq 'loginname is null...'}">   
				    <span class="red">用户名不可以为空!</span>  
				  </c:when> 
				  <c:when test="${SPRING_SECURITY_LAST_EXCEPTION.message eq 'loginname is null'}">   
				    <span class="red">密码不可以为空!</span> 
				  </c:when> 
				  <c:otherwise>   
