...
public abstract class AbstractStaticPlatformEnvironment
    ...
    protected void organizePlatformData() {
        ...
        for (IUnitInfo ui : CodeRepositoryCache.unitInfoRepo.getCachedTarget()) {
            UnitInfo unit = (UnitInfo)CodeRepositoryCache.codeToUnitMap.getCachedTarget().get(ui.getParentUnit());
            if (unit != null)
                unit.addSubUnit((UnitInfo)ui);
        }

        for (IUserUnit uu : CodeRepositoryCache.userUnitRepo.getCachedTarget()) {
            UserInfo user = (UserInfo)CodeRepositoryCache.codeToUserMap.getCachedTarget().get(uu.getUserCode());
            if (user != null)
                user.addUserUnit((UserUnit)uu);
            UnitInfo unit = (UnitInfo)CodeRepositoryCache.codeToUnitMap.getCachedTarget().get(uu.getUnitCode());
            if (unit != null)
