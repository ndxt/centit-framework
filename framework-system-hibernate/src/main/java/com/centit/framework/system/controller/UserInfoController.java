...
public class UserInfoController extends BaseController {
    ...
    public void list(String[] field, PageDesc pageDesc, String _search, HttpServletRequest request, HttpServletResponse response) {
        ...
        if("isValid".equals((String)searchColumn.get("sort"))) {
    ...
    public void create(@Valid UserInfo userInfo,@Valid UserUnit userunit,HttpServletRequest request, HttpServletResponse response) {
    	...
    	dbuserinfo=sysUserManager.getObjectByProperty("regEmail", userInfo.getRegEmail());
    	if(null!=dbuserinfo)
    	{
    		 JsonResultUtils.writeErrorMessageJson(
    				 ResponseData.ERROR_FIELD_INPUT_CONFLICT,
    				 "邮箱"+userInfo.getRegEmail()+"已存在，请更换！", response);
    		 return;
    	}
        ...
        if(userunit!=null){
        	...
        	userunit.setLastModifyDate(new Date());
        	userunit.setCreator(user.getUserCode());
    ...
    public void edit(@PathVariable String userCode, UserInfo userInfo,
    	...
    	if(sysUserManager.isLoginNameExist(userCode, userInfo.getLoginName()))
    	{
    		 JsonResultUtils.writeErrorMessageJson(
    				 ResponseData.ERROR_FIELD_INPUT_CONFLICT,
    				 "登录名"+userInfo.getLoginName()+"已存在，请更换！", response);
     	...
     	if(sysUserManager.isEmailExist(userCode,userInfo.getRegEmail()))
    	{
    		JsonResultUtils.writeErrorMessageJson(
    				ResponseData.ERROR_FIELD_INPUT_CONFLICT,
    				"邮箱"+userInfo.getRegEmail()+"已存在，请更换！", response);
    ...
    public void resetPwd(@PathVariable String userCode,
                         HttpServletRequest request,HttpServletResponse response) {
        sysUserManager.resetPwd(userCode);
